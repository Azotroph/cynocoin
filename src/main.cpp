...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Cynocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Cynocoin Signed Message:\n";
...
{
    // Cynocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 1 * COIN;
		...
		nSubsidy = diff * 15 * COIN; //Close to 0.1 Coins for each 1 MH/s on network

    if (nHeight == 1)
		nSubsidy + nFees = 2000000

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 24 * 60 * 60; // Cynocoin: 1 day
static const int64 nTargetSpacing = 80; // Cynocoin: 1 minute 20 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Cynocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
        static const int64 BlocksTargetSpacing = 60; // Cynocoin: 1 minute
        static const unsigned int TimeDaySeconds = 24 * 60 * 60; // Cynocoin: 1 day
...
{
    // Cynocoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
	...
	// Mainnet Genesis block:
		...
		const char* pszTimestamp = "28 March 2014. Bitcoin 2.0 Shows Technology Evolving Beyond Use as Money;
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // Cynocoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CynocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CynocoinMiner : ProcessBlock, block not accepted");
    }
